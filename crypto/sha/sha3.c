...
{
    return SHA3_absorb_12(ctx->A, inp, len, ctx->block_size);
}
...
int ossl_turboshake_final_default(KECCAK1600_CTX *ctx, unsigned char *out, size_t outlen)
{
    ...
    (void)SHA3_absorb_12(ctx->A, ctx->buf, bsz, bsz);
    SHA3_squeeze_12(ctx->A, out, outlen, bsz, 0);
    return 1;
}
...
int ossl_turboshake_squeeze_default(KECCAK1600_CTX *ctx, unsigned char *out, size_t outlen)
{
    ...
    if (outlen >= bsz) {
        len = bsz * (outlen / bsz);
        SHA3_squeeze_12(ctx->A, out, len, bsz, next);
        next = 1;
        out += len;
        outlen -= len;
    }
    if (outlen > 0) {
        /* Step 3. Squeeze one more block into a buffer */
        SHA3_squeeze_12(ctx->A, ctx->buf, bsz, bsz, next);
        memcpy(out, ctx->buf, outlen);
        /* Step 4. Remember the leftover part of the squeezed block */
        ctx->bufsz = bsz - outlen;
    }
